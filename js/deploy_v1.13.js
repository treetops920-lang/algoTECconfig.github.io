...
const FW_GRACE_SECONDS = 180; // Firmware needs more time
...
function isVersionOlder(current, target) {
    ...
    for (let i = 0; i < Math.max(c.length, t.length); i++) {
        const cP = c[i] || 0;
        const tP = t[i] || 0;
        if (tP > cP) return true;  // Target is newer
        if (cP > tP) return false; // Device is already newer/same
    }
    return false;
}
...
function needsFirmwareUpdate(info) {
    ...
    if (!fw) return null;
    // Only returns the firmware object if the device is OLDER than the map
...
async function waitForDevice(ip, customGrace) {
    ...
    action(`Waiting ${grace}s for reboot (${ip})`);
    ...
    while (Date.now() - start < timeoutMs) {
        try {
            ...
            // Visual feedback: . is connection refused, ? is auth/time mismatch
            const mark = e.response?.status === 403 ? "?" : ".";
            process.stdout.write(`${C.GRAY}${mark}${C.RESET}`);
...
async function processDevice(oldIp, newIp) {
    ...
    try {
        const infoAbout = await getFirmware(oldIp);
        ok(`${infoAbout.model} detected (FW ${infoAbout.version})`);
        ...
        // 2. FIRMWARE (Check and Update only if older)
        ...
        if (fwUpdate && !DRY_RUN) {
            ...
            // Longer wait for firmware flashing
            await waitForDevice(newIp, FW_GRACE_SECONDS);
        } else {
            ok(`Firmware version ${stableInfo.version} is sufficient. Skipping update.`);
        }
