...
async function apiRequestWithRetry(host, endpoint, method, data = null) {
    try {
        ...
        if (e.response?.status === 403 && e.response.headers?.date) {
            warn(`Time drift detected for ${host}, retrying`);
...
async function waitForDevice(ip) {
    action(`Waiting ${GRACE_SECONDS}s for reboot (${ip})`);
